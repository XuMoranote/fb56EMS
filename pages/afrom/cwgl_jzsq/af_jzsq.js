// pages/afrom/cwgl_jzsq/af_jzsq.js
const app = getApp();
...
Page({
  ...
  data: {
    //工作流类型
    gzllx_index: 0,
    gzllx_array: ['请选择', '个人借支工作流', '工伤借款申请工作流', '备用金申请工作流'],
    gzllx2_index: 0,
    gzllx2_array: [],
    gzllx3_index: 0,
    gzllx3_array: [],
  ...
  onLoad: function (options) {
    let data = '<div><h4>申请原因填写规则:</h4><div>支付+XX省区（总部）+XX部门+XX人名+报销+X年X月X日+费用名称</div><div><h4>字段提示：</h4></div><div style="color: red;">1.个人借支提示：请在最晚1个月之内进行核销（逾期会有负激励 ）</div><div style="color: red;">2.工伤借支：请在最晚3个月90天内核销（逾期会产生负激励）</div></div>';
  ...
  onSubmit(e) {
    ...
    if (self.data.formData.gzllx == '工伤借款申请工作流') {
      if (util.strIsEmpty(self.data.formData.gsygxm) || util.strIsEmpty(self.data.formData.gsyggh)) {
        self.setData({
          error: '请输入工伤员工姓名和工伤员工工号!'
        });
        return;
      }
    }
    ...
    //获取菜单ID
    var menuid = munePage.GetMenuCode("借支申请");
    if (Number(menuid) <= 0) {
      self.setData({
        error: '获取菜单ID异常！'
      });
    ...
    let paramData = JSON.stringify({
      ...
      // "_flowid": "226054cb-51ab-4707-a5ca-affb19588a5c",
      "SubTitle": "FB-JZSQD-",
      ...
      "NameOfInjuredEmployee": self.data.formData.gsygxm, //工伤员工姓名
      "WorkNOOfInjuredEmployee": self.data.formData.gsyggh, //工伤员工工号
