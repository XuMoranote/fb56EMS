// pages/afrom/cwgl_jylfybzd/af_jylfybzd.js
const app = getApp();
...
Page({
  ...
  data: {
    //工作流类型
    gzllx_index: 0,
    gzllx_array: ['请选择', '外请车提送货成本申请工作流', '干线结算申请工作流', '支线结算申请工作流'],
    gzllx2_index: 0,
    gzllx2_array: [],
    gzllx3_index: 0,
    gzllx3_array: [],
  ...
  onSubmit(e) {
    ...
    //获取菜单ID
    var menuid = munePage.GetMenuCode("经营类费用报账单");
    if (Number(menuid) <= 0) {
      self.setData({
        error: '获取菜单ID异常！'
      });
    ...
    let paramData = JSON.stringify({
      ...
      // "_flowid": "93625e4a-11f5-4faa-9c1b-dbbca877aeac",
      "SubTitle": "FB-JYLFYBZD-",
