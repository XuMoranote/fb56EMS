// pages/afrom/cwgl_cdyfkbzd/af_cdyfkbzd.js
const app = getApp();
...
Page({
  ...
  data: {
    //工作流类型
    gzllx_index: 0,
    gzllx_array: ['请选择', '网点预付款申请工作流', '分拨场地预付款申请工作流', '办公/宿舍场地预付款申请工作流'],
    gzllx2_index: 0,
    gzllx2_array: [],
    gzllx3_index: 0,
    gzllx3_array: [],
  ...
  bindPickerChange_gzllx2(e) {
    ...
    this.data._waydata.forEach(element => {
      element.children.forEach(item => {
        if (item.text == gzllx2 && !util.isInArray(gzllx3,item.text)) {          
          gzllx3.push('请选择');
  ...
  */
  ...
  onSubmit(e) {
    ...
    //获取菜单ID
    var menuid = munePage.GetMenuCode("场地预付款报账单");
    if (Number(menuid) <= 0) {
      self.setData({
        error: '获取菜单ID异常！'
      });
    ...
    let paramData = JSON.stringify({
      ...
      // "_flowid": "9355aa76-ab11-4c45-a362-ab208a4b83cc",
      "SubTitle": "FB-CDYFKBZD-",
