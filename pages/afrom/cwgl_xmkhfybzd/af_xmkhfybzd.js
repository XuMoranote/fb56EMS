// pages/afrom/cwgl_xmkhfybzd/af_xmkhfybzd.js
const app = getApp();
...
Page({
  ...
  data: {
    //工作流类型
    gzllx_index: 0,
    gzllx_array: ['请选择', '项目客户申请工作流', '项目客户预付款申请工作流'],
    gzllx2_index: 0,
    gzllx2_array: [],
    gzllx3_index: 0,
    gzllx3_array: [],
  ...
  onSubmit(e) {
    ...
    //获取菜单ID
    var menuid = munePage.GetMenuCode("项目客户费用报账单");
    if (Number(menuid) <= 0) {
      self.setData({
        error: '获取菜单ID异常！'
      });
    ...
    let paramData = JSON.stringify({
      ...
      // "_flowid": "2176a185-e720-4f87-b92b-6868f5e48343",
      "SubTitle": "FB-XMKHFYBZD-",
