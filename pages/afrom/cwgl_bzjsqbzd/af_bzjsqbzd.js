// pages/afrom/cwgl_bzjsqbzd/af_bzjsqbzd.js
const app = getApp();
...
Page({
  ...
  data: {
    //工作流类型
    gzllx_index: 0,
    gzllx_array: ['请选择', '保证金/押金申请工作流', '项目客户保证金/押金申请工作流'],
    gzllx2_index: 0,
    gzllx2_array: [],
    gzllx3_index: 0,
    gzllx3_array: [],
  ...
  onSubmit(e) {
    ...
    //获取菜单ID
    var menuid = munePage.GetMenuCode("保证金申请报账单");
    if (Number(menuid) <= 0) {
      self.setData({
        error: '获取菜单ID异常！'
      });
    ...
    let paramData = JSON.stringify({
      ...
      // "_flowid": "ba0ecc1f-f0bb-491b-b05e-49e4f5645c58",
      "SubTitle": "FB-BZJSQBZD-",
